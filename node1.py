...
def Process3_client(receive_port, sender_port):
    global data
    ...
    while(True):
        if data:
            print('Received the data TC: ', data)
    ...
    t1.join()
    print('>>>>>>>>>>>>>>>>>>')
    print('transaction states at node1')
    print(transaction)
    client_socket.close()
